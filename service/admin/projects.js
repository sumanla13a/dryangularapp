...
var user = {
  find: function (req, res, next) {
    ...
    if (req.params.userId) {
      filters.user_id = req.params.userId;
    }
    ...
    req.app.db.models.Project.pagedFind({
      filters: filters,
  ...
  create: function (req, res, next) {
    ...
    workflow.on('validate', function () {
      if (!req.body.user_id) {
        workflow.outcome.errors.push('Please login first.');
    ...
    workflow.on('duplicateUsernameCheck', function () {
      req.app.db.models.Project.findOne({project_name: req.body.project_name, _id: req.body.user_id}, function (err, user) {
        if (err) {
          return workflow.emit('exception', err);
        }

        if (user) {
          workflow.outcome.errors.push('That project already exists.');
          return workflow.emit('response');
        }
    ...
    workflow.on('createProject', function () {
      ...
      req.app.db.models.Project.create(fieldsToSet, function (err, project) {
        if (err) {
          return workflow.emit('exception', err);
        }
        console.log('creating user folder ' + user._id)
        var mkdirp = require('mkdirp');
        mkdirp(appRoot + '/tmp/' + fieldsToSet.user_id + '/' + fieldsToSet.project_name, function(err) { 
          if(err) {
            console.log('err creating folder');
          }
        });

        workflow.outcome.record = project;
        return workflow.emit('response');
      });
    });
